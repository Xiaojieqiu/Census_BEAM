...
mc_AT12_cds_subset_all_gene@reducedDimS <- AT12_cds_subset_all_gene@reducedDimS
mc_AT12_cds_subset_all_gene@reducedDimA <- AT12_cds_subset_all_gene@reducedDimA
mc_AT12_cds_subset_all_gene@reducedDimK <- AT12_cds_subset_all_gene@reducedDimK
mc_AT12_cds_subset_all_gene@reducedDimW <- AT12_cds_subset_all_gene@reducedDimW
mc_AT12_cds_subset_all_gene@minSpanningTree <- AT12_cds_subset_all_gene@minSpanningTree

mc_AT12_cds_subset_all_gene@dim_reduce_type <- "ICA" #set dim_reduce_type to "ICA"
...
plot_spanning_tree(mc_AT12_cds_subset_all_gene, color_by="State", show_backbone=T, backbone_color = 'black', #show_all_lineages = F,
                   markers=NULL, show_cell_names = F, cell_link_size = 0.2) + scale_y_reverse() + 
  scale_size(range = c(0.1, 2.5)) + nm_theme()
...
plot_genes_branched_pseudotime2(mc_AT12_cds_subset_all_gene[example_ids, ], cell_color_by = "Time", trajectory_color_by = "Lineage", fullModelFormulaStr = '~sm.ns(Pseudotime, df = 3)*Lineage', normalize = F, stretch = T, lineage_labels = c('AT1', 'AT2'), cell_size = 1, ncol = 2) + nm_theme()+ ylab('Transcript counts') + xlab('Pseudotime')
dev.off()
...
mc_house_keeping_marker_branchTest_res <- branchTest(mc_AT12_cds_subset_all_gene[fig2_genes_ids, ], cores = 1, relative_expr = F, weighted = T)
mc_house_keeping_marker_branchTest_res[fig2_genes_ids, 'pval']

pdf('./main_figures/fig2b.pdf', width = 2.6, height = 1.75)
plot_genes_branched_pseudotime2(mc_AT12_cds_subset_all_gene[fig2_genes_ids, ], color_by = "State", panel_order = fig2_genes, 
...
mc_AT12_cds_subset_all_gene_ILRs_list$str_raw_div_df <- mc_AT12_cds_subset_all_gene_ILRs_list$str_branchA_expression_curve_matrix - mc_AT12_cds_subset_all_gene_ILRs_list$str_branchB_expression_curve_matrix
mc_AT12_cds_subset_all_gene_ILRs_list$str_norm_div_df <- mc_AT12_cds_subset_all_gene_ILRs_list$str_raw_div_df / rMax(mc_AT12_cds_subset_all_gene_ILRs_list$str_raw_div_df) #calculate normalized divergence
mc_AT12_cds_subset_all_gene_ILRs_list$log_str_raw_div_df <- log2((mc_AT12_cds_subset_all_gene_ILRs_list$str_branchA_expression_curve_matrix + .1)/(mc_AT12_cds_subset_all_gene_ILRs_list$str_branchB_expression_curve_matrix + .1))
mc_AT12_cds_subset_all_gene_ILRs_list$norm_str_logfc_df <- mc_AT12_cds_subset_all_gene_ILRs_list$str_logfc_df / rMax(mc_AT12_cds_subset_all_gene_ILRs_list$log_str_raw_div_df)

new_cds <- buildLineageBranchCellDataSet(AT12_cds_subset_all_gene, stretch=T)

MST_branch_range <- range(pData(AT12_cds_subset_all_gene[, pData(AT12_cds_subset_all_gene)$State == 1])[, 'Pseudotime'])
test <- (pData(AT12_cds_subset_all_gene)$Pseudotime - MST_branch_range[2])
first_branching_cell <- colnames(AT12_cds_subset_all_gene)[which(abs(test) == min(abs(test)))]
MST_branch_time <- round(pData(new_cds)[first_branching_cell, 'Pseudotime'])
all_mc_bifurcation_time <- detectBifurcationPoint(mc_AT12_cds_subset_all_gene_ILRs_list$norm_str_logfc_df[, MST_branch_time:100])

bif_time <- all_mc_bifurcation_time[markers]
...
plot_genes_branched_pseudotime2(mc_AT12_cds_subset_all_gene[example_ids, ], color_by = "State", panel_order = markers, trajectory_color_by = 'Lineage', 
...
mc_str_logfc_df_list <- calILRs(cds = mc_AT12_cds_subset_all_gene[add_quake_gene_all_marker_ids, ], lineage_states = c(2, 3), stretch = T,cores = 1, 
...
mc_bifurcation_time <- detectBifurcationPoint(mc_str_logfc_df_list$str_norm_div_df[, ], ILRs_threshold = 0.3) #return the inflection point
names(mc_bifurcation_time) <- fData(mc_AT12_cds_subset_all_gene[row.names(mc_str_logfc_df_list$str_norm_div_df), ])$gene_short_name
mc_valid_bifurcation_time  <- mc_bifurcation_time[!is.na(mc_bifurcation_time)]
mc_valid_bifurcation_time <- mc_valid_bifurcation_time[unique(names(mc_valid_bifurcation_time))]

mc_bif_df <- data.frame(bifurcation_time_point = mc_bifurcation_time[c(AT1_early, AT1_late, AT2_early, AT2_late)], 
...
mc_marker_genes_branch_pval <- branchTest(mc_AT12_cds_subset_all_gene[add_quake_gene_all_marker_ids, ], cores = 1, relative_expr = T, weighted = F)

valid_timing_id <- timing_example_ids[mc_marker_genes_branch_pval[timing_example_ids, 'qval'] < 0.01]

mc_bif_df <- mc_bif_df[!is.na(mc_bif_df$bifurcation_time_point), ]
data <- subset(mc_bif_df, abs(bifurcation_time_point) > 27)
...
#figure 2d: 
add_quake_gene_all_marker_ids_branchTest_res <- weihgted_mc_AT12_cds_subset_all_gene[add_quake_gene_all_marker_ids, ]
...
pdf(paste(submission_directory, 'tmp/submission_fig2d.pdf', sep = ''))
valid_add_quake_gene_all_marker_ids_res_no_fit_log <- plot_genes_branched_heatmap(mc_AT12_cds_subset_all_gene[add_quake_gene_all_marker_ids, ], num_clusters=4, norm_method = "log", use_fitting_curves = F, scaling = F, hmcols = hmcols, return_all = T)
dev.off()

pdf(paste(submission_directory, 'tmp/submission_fig2d.1.pdf', sep = ''))
valid_add_quake_gene_all_marker_ids_res_FIT_log <- plot_genes_branched_heatmap(mc_AT12_cds_subset_all_gene[add_quake_gene_all_marker_ids, ], num_clusters=4, norm_method = "log", use_fitting_curves = T, scaling = F, hmcols = hmcols, return_all = T)
dev.off()
...
cell_cycle_timing_example_ids_df <- weihgted_mc_AT12_cds_subset_all_gene[c(timing_example_ids, cell_cycle_markers_id), c('gene_short_name', 'pval')]
...
#fig 3g:
Time <- pData(mc_AT12_cds_subset_all_gene)$Time
...
mc_E14.5_cell <- rowMeans(exprs(mc_adj_cds[, colnames(mc_AT12_cds_subset_all_gene)[which(Time == 'E14.5')]]))
mc_E16.5_cell <- rowMeans(exprs(mc_adj_cds[, colnames(mc_AT12_cds_subset_all_gene)[which(Time == 'E16.5')]]))
mc_E18.5_cell <- rowMeans(exprs(mc_adj_cds[, colnames(mc_AT12_cds_subset_all_gene)[which(Time == 'E18.5')]]))
mc_Adult_cell <- rowMeans(exprs(mc_adj_cds[, colnames(mc_AT12_cds_subset_all_gene)[which(Time == 'Adult')]]))
...
# panel b: 
branch_gene_str_norm_div_df <- mc_AT12_cds_subset_all_gene_ILRs_list$str_logfc_df[row.names(weihgted_mc_AT12_cds_subset_all_gene[weihgted_mc_AT12_cds_subset_all_gene[, 'qval'] <= 0.05, ]), ]
quake_branch_genes <- row.names(subset(weihgted_mc_AT12_cds_subset_all_gene, qval <= 0.05))

fData(mc_AT12_cds_subset_all_gene)$num_cell_expressed <- esApply(mc_AT12_cds_subset_all_gene[, ], 1, function(x) sum(round(x) > 0))
valid_expressed_genes <- row.names(subset(fData(mc_AT12_cds_subset_all_gene), num_cell_expressed > 5))

pdf(paste(submission_directory, 'main_figures/fig4b.pdf', sep = ''), height = 2.45, width = 3.4)
all_AT12_heatmap <- plot_genes_branched_heatmap(mc_AT12_cds_subset_all_gene[quake_branch_genes[quake_branch_genes %in% valid_expressed_genes], ], cores = detectCores(), 
                                                stretch = T, return_all = T)
dev.off()

clusters <- as.numeric(all_AT12_heatmap$annotation_row$Cluster); 
names(clusters) <- fData(mc_AT12_cds_subset_all_gene[row.names(all_AT12_heatmap$annotation_row), ])$gene_short_name

branchGenes_gsa_results_branch_genes <- collect_gsa_hyper_results(mc_AT12_cds_subset_all_gene[, ], mouse_go_gsc, clusters)

pdf(paste(submission_directory, "./main_figures/fig4b_go_enrichment.pdf", sep = ''), height=100, width=15)
plot_gsa_hyper_heatmap(mc_AT12_cds_subset_all_gene, branchGenes_gsa_results_branch_genes, significance = 1e-2)
dev.off()
...
valid_cell_genes_TF_enrichment_results_5k <- collect_gsa_hyper_results(mc_AT12_cds_subset_all_gene[valid_gene_id_cell_new, ], lung_TF_5k_enrichment_gsc, clusters)
hyper_df <- plot_gsa_hyper_heatmap(mc_AT12_cds_subset_all_gene[valid_gene_id_cell_new, ], valid_cell_genes_TF_enrichment_results_5k, significance = 1e-2)
...
select_branch_motif_Tfs_id <- row.names(subset(fData(mc_AT12_cds_subset_all_gene), toupper(gene_short_name) %in% toupper(select_genes))) 
