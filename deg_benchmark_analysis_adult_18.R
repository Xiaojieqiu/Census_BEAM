...
#update DESeq 1: 
DESeq1_test <- function (count_d, pd = pData(new_abs_cds_Adult_18[, colnames(count_d)]), condition = Time_ori, disp_method = "blind",
    sharing = "fit-only", test_type = "nbinomGLMTest", scale = F,
    contrast = c("Adult", "E18.5"), fullModelFormulaStr = "count~condition",
...
#double check the cells selected are the same as we used in the paper:  identical(colnames(new_abs_cds_Adult_18), cells_used)
new_abs_cds_Adult_18 <- absolute_cds[, intersect(valid_cells, row.names(subset(pData(absolute_cds), Time %in% c('E18.5', 'Adult'))))]
new_abs_cds_Adult_18 <- estimateSizeFactors(new_abs_cds_Adult_18) #calculate the size factor for performing the relative absolute expression tests
 
table(pData(new_abs_cds_Adult_18)$Time) #check the number of used cells at each Time Point

new_mc_cds_Adult_18 <- mc_adj_cds[, colnames(new_abs_cds_Adult_18)]
new_mc_cds_Adult_18 <- estimateSizeFactors(new_mc_cds_Adult_18)

new_std_cds_Adult_18 <- standard_cds[, colnames(new_abs_cds_Adult_18)]
# new_std_cds_Adult_18 <- estimateSizeFactors(new_std_cds_Adult_18) #FPKM values are already on relative scale

#prepare the readcount data for DESeq / SCDE: 
read_countdata <- read_countdata[row.names(new_abs_cds_Adult_18), colnames(new_abs_cds_Adult_18)] 

#create a cds for readcount data to perform the default DEG tests for DESeq and SCDE : 
count_cds <- newCellDataSet(read_countdata[row.names(new_abs_cds_Adult_18), colnames(new_abs_cds_Adult_18)],
                            phenoData = new("AnnotatedDataFrame", data = pData(new_abs_cds_Adult_18)),
                            featureData = new("AnnotatedDataFrame", data = fData(new_abs_cds_Adult_18)),
...
# std_scde_res_list <- scde_DEG(dir = NULL, count_cds = count_cds, DEG_attribute = 'Time', contrast = c('Adult', 'E18.5'), n.cores = 1)
# save(std_scde_res_list, file = 'std_scde_res_list')

# std_scde_res_list_no_normalize <- scde_DEG(dir = NULL, count_cds = count_cds, DEG_attribute = 'Time', contrast = c('Adult', 'E18.5'), n.cores = 1)
# save(std_scde_res_list_no_normalize, file = 'std_scde_res_list_no_normalize')

# #calculate the pval with the normalized transcripts with scde: 
# abs_scde_res_list <- scde_DEG(dir = NULL, count_cds = new_abs_cds_Adult_18, DEG_attribute = 'Time', contrast = c('Adult', 'E18.5'), n.cores = 1, normalize = T)
# save(abs_scde_res_list, file = 'abs_scde_res_list')

# abs_scde_res_list_no_normalize <- scde_DEG(dir = NULL, count_cds = new_abs_cds_Adult_18, DEG_attribute = 'Time', contrast = c('Adult', 'E18.5'), n.cores = 1)
# save(abs_scde_res_list_no_normalize, file = 'abs_scde_res_list_no_normalize')

# mc_scde_res_list_no_normalize <- scde_DEG(dir = NULL, count_cds = new_mc_cds_Adult_18, DEG_attribute = 'Time', contrast = c('Adult', 'E18.5'), n.cores = 1, normalize = F)
# save(mc_scde_res_list_no_normalize, file = 'mc_scde_res_list_no_normalize')

# mc_scde_res_list <- scde_DEG(dir = NULL, count_cds = new_mc_cds_Adult_18, DEG_attribute = 'Time', contrast = c('Adult', 'E18.5'), n.cores = 1, normalize = T)
# save(mc_scde_res_list, file = 'mc_scde_res_list')

load('std_scde_res_list')
load('std_scde_res_list_no_normalize')
load('abs_scde_res_list')
load('abs_scde_res_list_no_normalize')
load('mc_scde_res_list_no_normalize')
load('mc_scde_res_list')

#load all other necessary packages: 
load_all_libraries()

# # #perform  the stastical tests on the data: 
new_std_diff_test_res <- differentialGeneTest(new_std_cds_Adult_18[1:transcript_num, ], 
                                              fullModelFormulaStr = "~Time", 
                                              reducedModelFormulaStr = "~1", cores = detectCores(), relative = F)  

new_std_diff_test_res_relative <- differentialGeneTest(new_std_cds_Adult_18[1:transcript_num, ], 
                                              fullModelFormulaStr = "~Time", 
                                              reducedModelFormulaStr = "~1", cores = detectCores(), relative = T)  


new_size_norm_abs_diff_test_res <- differentialGeneTest(new_abs_cds_Adult_18[1:transcript_num, ], 
                                                        fullModelFormulaStr = "~Time", 
                                                        reducedModelFormulaStr = "~1", cores = detectCores(), relative = T)

new_size_norm_mc_diff_test_res <- differentialGeneTest(new_mc_cds_Adult_18[1:transcript_num, ], 
                                                       fullModelFormulaStr = "~Time", 
                                                       reducedModelFormulaStr = "~1", cores = detectCores(), relative = T)
...
Time_ori <- pData(new_abs_cds_Adult_18)$Time #Time as input for newCountDataSet
...
abs_sf_mat <- matrix(rep(sizeFactors(new_abs_cds_Adult_18), nrow(new_abs_cds_Adult_18)), nrow = nrow(new_abs_cds_Adult_18), byrow = T)
mc_sf_mat <- matrix(rep(sizeFactors(new_mc_cds_Adult_18), nrow(new_mc_cds_Adult_18)), nrow = nrow(new_mc_cds_Adult_18), byrow = T)
...
abs_count_d <- newCountDataSet(round(t(t(exprs(new_abs_cds_Adult_18)) )), (Time_ori)) #Don't normalized the data by size factor
# abs_dtable_pool_max_nbinomTest <- DESeq1_test(abs_count_d, pd = pData(new_abs_cds_Adult_18), disp_method = 'pooled', sharing = 'maximum', test_type = 'nbinomTest') 
...
abs_dtable_pool_max_nbinomGLMTest <- DESeq1_test(abs_count_d[1:transcript_num, ], pd = pData(new_abs_cds_Adult_18), disp_method = 'pooled', sharing = 'maximum', test_type = 'nbinomGLMTest', scale = T) 
row.names(abs_dtable_pool_max_nbinomGLMTest$dtalbe) <- row.names(new_abs_cds_Adult_18[1:transcript_num, ])

#permutation results for two group test on the FPKM value (gold standard for monocle on FPKM values with tobit model): 
Time_order <- order(Time_ori) #order the Adult cell at the begining (ensure Adult cells number is 43 while E18.5 is 74)
std_split_cds <- split(t(exprs(new_std_cds_Adult_18[1:transcript_num, Time_order])), col(t(exprs(new_std_cds_Adult_18[1:transcript_num, Time_order])), as.factor = T))
std_fc <- esApply(new_std_cds_Adult_18[1:transcript_num, ], 1, mean_fc, grp0 = 'Adult', grp1 = 'E18.5')
std_split_fc <- split(t(std_fc), col(t(std_fc), as.factor = T))

permuation_pval <- function (x, fc, alpha = table(pData(new_abs_cds_Adult_18)$Time)['Adult'], beta = table(pData(new_abs_cds_Adult_18)$Time)['E18.5'], permutate_num = 10000,
    return_fc = F)
{
...
#readcount_fc <- esApply(count_cds[1:transcript_num, ], 1, mean_fc, grp0 = 'Adult', grp1 = 'E18.5') #valid_gene_id
...
readcount_fc <- apply(round(exprs(count_cds)[1:transcript_num, Time_order] / readcount_sf_mat[1:transcript_num, Time_order]), 1, mean_fc, grp0 = 'Adult', grp1 = 'E18.5', grp = pData(count_cds[1:transcript_num, Time_order])$Time) #valid_gene_id
...
# # readcount_permutate_pval <- permu_two_group_gen(count_cds[, Time_order], alpha = 43, beta = 60, grp0 = 'Adult', grp1 = 'E18.5', group = pData(count_cds)$Time)
# #mode_size_norm_permutate_ratio_by_geometric_mean <- cal_perm_pval_size_norm(new_abs_cds_Adult_18[1:transcript_num, Time_order])
# #mc_mode_size_norm_permutate_ratio_by_geometric_mean <- cal_perm_pval_size_norm(new_mc_cds_Adult_18[1:transcript_num, Time_order])

# # new implementation with size factor normalization: 
abs_split_cds <- split(round(t(exprs(new_abs_cds_Adult_18[1:transcript_num, Time_order]) / abs_sf_mat[1:transcript_num, Time_order])), col(t(exprs(new_abs_cds_Adult_18[1:transcript_num, Time_order])), as.factor = T))
abs_fc <- apply(round(exprs(new_abs_cds_Adult_18)[1:transcript_num, Time_order] / abs_sf_mat[1:transcript_num, Time_order]), 1, mean_fc, grp0 = 'Adult', grp1 = 'E18.5', grp = pData(new_abs_cds_Adult_18[1:transcript_num, Time_order])$Time) #valid_gene_id
...
mc_split_cds <- split(round(t(exprs(new_mc_cds_Adult_18[1:transcript_num, Time_order]) / mc_sf_mat[1:transcript_num, Time_order])), col(t(exprs(new_mc_cds_Adult_18[1:transcript_num, Time_order])), as.factor = T))
mc_fc <- apply(round(exprs(new_mc_cds_Adult_18)[1:transcript_num, Time_order] / mc_sf_mat[1:transcript_num, Time_order]), 1, mean_fc, grp0 = 'Adult', grp1 = 'E18.5', grp = pData(new_mc_cds_Adult_18[1:transcript_num, Time_order])$Time) #valid_gene_id
...
abs_edgeR_res <- edgeR_test(exprs(new_abs_cds_Adult_18[1:transcript_num, ]), group = Time_ori, glm = F)

edgeR_res_glm <- edgeR_test(counts = exprs(count_cds)[1:transcript_num, ], glm = T)
abs_edgeR_res_glm <- edgeR_test(exprs(new_abs_cds_Adult_18[1:transcript_num, ]), group = Time_ori, glm = T)
  
deseq2_res <- DESeq2_deg(dir = NULL, count_cds[1:transcript_num, ], Time = Time_ori, pd = pData(count_cds))
abs_deseq2_res <- DESeq2_deg(dir = NULL, new_abs_cds_Adult_18[1:transcript_num, ], Time = Time_ori, pd = pData(new_abs_cds_Adult_18))

#add the analysis for the mc counts: 
new_mc_cds_Adult_18_d <- newCountDataSet(round(exprs(new_mc_cds_Adult_18))[, ], Time_ori) 
mc_dtable_pool_max_nbinomGLMTest <- DESeq1_test(new_mc_cds_Adult_18_d[1:transcript_num, ], pd = pData(new_mc_cds_Adult_18), disp_method = 'pooled', sharing = 'maximum', test_type = 'nbinomGLMTest', scale = T) 
row.names(mc_dtable_pool_max_nbinomGLMTest$dtalbe) <-  row.names(new_mc_cds_Adult_18[1:transcript_num, ])
mc_edgeR_res_glm <- edgeR_test(exprs(new_mc_cds_Adult_18[1:transcript_num, ]), group = Time_ori, glm = T)
mc_edgeR_res <- edgeR_test(exprs(new_mc_cds_Adult_18[1:transcript_num, ]), group = Time_ori, glm = F)
mc_deseq2_res <- DESeq2_deg(dir = NULL, new_mc_cds_Adult_18[1:transcript_num, ], Time = Time_ori, pd = pData(new_mc_cds_Adult_18))
...
# monocle_p[1:5]
# monocle_p_readcount[1:5]
# new_abs_size_norm_monocle_p_ratio_by_geometric_mean[1:5]
# new_mc_size_norm_monocle_p_ratio_by_geometric_mean[1:5]
# default_edgeR_p[1:5]
# abs_default_edgeR_p[1:5]
# default_deseq2_p[1:5]
# default_deseq_p[1:5]
# abs_default_deseq_p[1:5]
# scde_p[1:5]
# abs_scde_p[1:5]
...
pval_mean <- data.frame(std = std_permutate_pval, abs = mode_size_norm_permutate_ratio_by_geometric_mean, 
                        ...
                        fpkm_mean = apply(new_std_cds_Adult_18[1:transcript_num, ], 1, mean), 
                        spike_mean = apply(new_abs_cds_Adult_18[1:transcript_num, ], 1, mean), 
                        mc_mean = apply(new_mc_cds_Adult_18[1:transcript_num, ], 1, mean), 
...
# lung_TPM_mat_Adult_18 <- exprs(TPM_cds[row.names(new_abs_cds_Adult_18), colnames(new_abs_cds_Adult_18)]) * median(pData(new_abs_cds_Adult_18)$Total_mRNAs) / 10^6

# lung_TPM_count_cds_Adult_18 <- newCellDataSet(lung_TPM_mat_Adult_18,
#                             phenoData = new("AnnotatedDataFrame", data = pData(new_abs_cds_Adult_18)),
#                             featureData = new("AnnotatedDataFrame", data = fData(new_abs_cds_Adult_18)),
#                             expressionFamily = negbinomial(),
#                             lowerDetectionLimit = 1)
# lung_TPM_count_cds_Adult_18 <- estimateSizeFactors(lung_TPM_count_cds_Adult_18)
# lung_TPM_count_cds_Adult_18 <- estimateDispersions(lung_TPM_count_cds_Adult_18)

# lung_TPM_count_scde_res_list <- scde_DEG(dir = NULL, count_cds = lung_TPM_count_cds_Adult_18, DEG_attribute = 'Time', contrast = c('Adult', 'E18.5'), n.cores = 1)
# lung_TPM_count_diff_test_res <- differentialGeneTest(lung_TPM_count_cds_Adult_18[1:transcript_num, ], 
#                                                         fullModelFormulaStr = "~Time", 
#                                                         reducedModelFormulaStr = "~1", cores = detectCores(), relative = T)

# Time_order <- order(Time_ori) #order the Adult cell at the begining (ensure Adult cells number is 43 while E18.5 is 74)

# lung_TPM_count_d <- newCountDataSet(round(lung_TPM_mat_Adult_18)[, ], Time_ori) 
# lung_TPM_count_dtable_pool_max_nbinomGLMTest <- DESeq1_test(lung_TPM_count_d[1:transcript_num, ], pd = pData(lung_TPM_mat_Adult_18), disp_method = 'pooled', sharing = 'maximum', test_type = 'nbinomGLMTest', scale = T) 
# row.names(lung_TPM_count_dtable_pool_max_nbinomGLMTest$dtalbe) <-  row.names(lung_TPM_count_d[1:transcript_num, ])

# permutation results for two group test on the FPKM value (gold standard for monocle on FPKM values with tobit model): 
# TPM_split_cds <- split(t(exprs(lung_TPM_count_cds_Adult_18[1:transcript_num, Time_order])), col(t(exprs(lung_TPM_count_cds_Adult_18[1:transcript_num, Time_order])), as.factor = T))
# TPM_fc <- esApply(lung_TPM_count_cds_Adult_18[1:transcript_num, Time_order], 1, mean_fc, grp0 = 'Adult', grp1 = 'E18.5', grp = pData(lung_TPM_count_cds_Adult_18[1:transcript_num, Time_order])$Time)
...
# TPM_sf_mat <- matrix(rep(sizeFactors(lung_TPM_count_cds_Adult_18), nrow(lung_TPM_count_cds_Adult_18)), nrow = nrow(lung_TPM_count_cds_Adult_18), byrow = T)
# TPM_split_cds <- split(t(round(exprs(lung_TPM_count_cds_Adult_18[1:transcript_num, Time_order]) / TPM_sf_mat[1:transcript_num, Time_order])), col(t(exprs(lung_TPM_count_cds_Adult_18[1:transcript_num, Time_order])), as.factor = T))
# TPM_round_fc <- apply(round(exprs(lung_TPM_count_cds_Adult_18[1:transcript_num, Time_order]) / TPM_sf_mat[1:transcript_num, Time_order]), col(t(exprs(lung_TPM_count_cds_Adult_18[1:transcript_num, Time_order]))), 1, mean_fc, grp0 = 'Adult', grp1 = 'E18.5', pData(lung_TPM_count_cds_Adult_18[1:transcript_num, Time_order])$Time) #valid_gene_id
...
# lung_TPM_count_edgeR_res <- edgeR_test(counts = exprs(lung_TPM_count_cds_Adult_18)[1:transcript_num, ], glm = F)
# lung_TPM_count_deseq2_res <- DESeq2_deg(dir = NULL, lung_TPM_count_cds_Adult_18[1:transcript_num, ], Time = Time_ori, pd = pData(count_cds))
...
# no_sf_mode_size_norm_permutate_ratio_by_geometric_mean <- cal_perm_pval_size_norm(new_abs_cds_Adult_18[1:transcript_num, Time_order])
# no_sf_mc_mode_size_norm_permutate_ratio_by_geometric_mean <- cal_perm_pval_size_norm(new_mc_cds_Adult_18[1:transcript_num, Time_order])

# mc_sf_mat <- matrix(rep(sizeFactors(new_mc_cds_Adult_18), nrow(new_mc_cds_Adult_18)), nrow = nrow(new_mc_cds_Adult_18), byrow = T)
# mc_split_cds <- split(t(exprs(new_mc_cds_Adult_18[1:transcript_num, Time_order]) / mc_sf_mat[1:transcript_num, Time_order]), col(t(exprs(new_mc_cds_Adult_18[1:transcript_num, Time_order])), as.factor = T))
# mc_fc <- apply(exprs(new_mc_cds_Adult_18)[1:transcript_num, Time_order] / mc_sf_mat[1:transcript_num, Time_order], 1, mean_fc, grp0 = 'Adult', grp1 = 'E18.5', grp = pData(new_mc_cds_Adult_18[1:transcript_num, Time_order])$Time) #valid_gene_id
# mc_split_fc <- split(t(mc_fc), col(t(mc_fc), as.factor = T))
# mc_mode_size_norm_permutate_ratio_by_geometric_mean <- mcmapply(permuation_pval, mc_split_cds, mc_split_fc, mc.cores = detectCores()) #multiple core
# closeAllConnections()
...
# no_sf_readcount_fc <- apply(exprs(count_cds[1:transcript_num, Time_order]), 1, mean_fc, grp0 = 'Adult', grp1 = 'E18.5', grp = pData(count_cds[, Time_order])$Time) #valid_gene_id
...
# TPM_cds_Adult_18 <- TPM_cds[, colnames(new_mc_cds_Adult_18)]
# #do the analysis for the TPM values: 
# no_sf_TPM_split_cds <- split(t(exprs(TPM_cds_Adult_18[1:transcript_num, Time_order])), col(t(exprs(TPM_cds_Adult_18[1:transcript_num, Time_order])), as.factor = T))
# no_sf_TPM_fc <- apply(exprs(TPM_cds_Adult_18[1:transcript_num, Time_order]), 1, mean_fc, grp0 = 'Adult', grp1 = 'E18.5', grp = pData(TPM_cds_Adult_18[, Time_order])$Time) #valid_gene_id
...
# TPM_split_cds <- split(t(exprs(TPM_cds_Adult_18[1:transcript_num, Time_order]) / readcount_sf_mat[1:transcript_num, Time_order]), col(t(exprs(TPM_cds_Adult_18[1:transcript_num, Time_order])), as.factor = T))
# TPM_fc <- apply(exprs(TPM_cds_Adult_18[1:transcript_num, Time_order] / readcount_sf_mat[1:transcript_num, Time_order]), 1, mean_fc, grp0 = 'Adult', grp1 = 'E18.5', grp = pData(TPM_cds_Adult_18[, Time_order])$Time) #valid_gene_id
...
# TPM_scde_res_list_no_normalize <- scde_DEG(dir = NULL, count_cds = TPM_cds_Adult_18, DEG_attribute = 'Time', contrast = c('Adult', 'E18.5'), n.cores = 1, normalize = F)
# new_tpm_cds_Adult_18_d <- newCountDataSet(round(exprs(TPM_cds_Adult_18))[, ], Time_ori) 
# tpm_dtable_pool_max_nbinomGLMTest <- DESeq1_test(TPM_cds_Adult_18[1:transcript_num, ], pd = pData(TPM_cds_Adult_18), disp_method = 'pooled', sharing = 'maximum', test_type = 'nbinomGLMTest', scale = T) 
# row.names(tpm_dtable_pool_max_nbinomGLMTest$dtalbe) <-  row.names(TPM_cds_Adult_18[1:transcript_num, ])
# tpm_edgeR_res_glm <- edgeR_test(exprs(TPM_cds_Adult_18[1:transcript_num, ]), group = Time_ori, glm = T)
# tpm_deseq2_res <- DESeq2_deg(dir = NULL, TPM_cds_Adult_18[1:transcript_num, ], Time = Time_ori, pd = pData(TPM_cds_Adult_18))
