...
# source ("isoform_support_functions.q")
...
HSMM_myo <- orderCells(HSMM_myo, reverse=T, num_paths=1)
...
actins <- HSMM_myo_isoform[row.names(subset(fData(HSMM_myo_isoform), 
                             gene_short_name %in% c("ACTA1", "ACTA2", "ACTB", "ACTG1", "ACTG2", "ACTC1") & num_cells_expressed >= 19)),]
#actins <- differentialGeneTest(actins)
pdf ("fig5c_remove_20ACTB.pdf", width=1.25, height=4.6)
plot_genes_in_pseudotime(actins[-10, ], 
...
pdf ("iso_switch_gsa_hyper_heatmap_go_bp.pdf", height=16, width=16)
plot_gsa_hyper_heatmap(HSMM_myo_isoform,  iso_switch_gsa_hyper_heatmap, significance=0.1)
dev.off()
...
iso_switch_heatmap <- plot_isoform_expression_heatmap(HSMM_myo_isoform, 
                                                     #c("TPM1", "TPM2"),
                                                     subset(iso_switch_test_res, qval < 0.1)$gene_short_name, 
                                                     num_clusters=num_iso_switch_clusters,
                                                     add_annotation_row=additional_row_annotation)
dev.off()
...
save.image('./RData/analysis_hsmm_isoform_census_analytical.RData')
