...
| gzip >$WORKING_DIR/kallisto-results/gene.tpm.gz

gunzip <$WORKING_DIR/kallisto-results/gene.tpm.gz \
| sort -k1,1 -k2,2 \
| /net/trapnell/vol1/jspacker/datamash/datamash -g 1,2 sum 3 \
| awk '{ printf "%s\t%s\t%.6g\n", $1, $2, $3; }' \
| gzip >$WORKING_DIR/kallisto-results/.gene.tpm.gz

mv $WORKING_DIR/kallisto-results/.gene.tpm.gz \
$WORKING_DIR/kallisto-results/gene.tpm.gz
