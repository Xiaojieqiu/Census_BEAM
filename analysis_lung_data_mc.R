...
pseudotime_mc_AT12_cds_subset <- differentialGeneTest(mc_AT12_cds_subset_all_gene[1:transcript_num, ], cores = detectCores(), relative_expr = F)
closeAllConnections()
branch_pseudotime_mc_AT12_cds_subset <- branchTest(mc_AT12_cds_subset_all_gene[1:transcript_num, ], fullModelFormulaStr = "~sm.ns(Pseudotime, df = 3)", reducedModelFormulaStr = "~1", cores = detectCores(), relative_expr = F, weighted = F)
closeAllConnections()

mc_adj_cds@dim_reduce_type <- 'ICA'
weihgted_mc_AT12_cds_subset_all_gene <- branchTest(mc_AT12_cds_subset_all_gene[1:transcript_num, ], cores = detectCores(), relative_expr = F, weighted = T)
closeAllConnections()
weihgted_mc_AT12_cds_subset_quake_gene <- branchTest(mc_AT12_cds_subset_all_gene[add_quake_gene_all_marker_ids, ], cores = detectCores(), relative_expr = F, weighted = T)
closeAllConnections()
...
save.image('./RData/analysis_lung_data_mc.RData')

####################################################################################################################################################################################################################

########################################FPKM value based results: ########################################
pseudotime_std_AT12_cds_subset <- differentialGeneTest(standard_cds[1:transcript_num, ], cores = detectCores(), relative_expr = F)
closeAllConnections()
branch_pseudotime_std_AT12_cds_subset <- branchTest(standard_cds[1:transcript_num, ], fullModelFormulaStr = "~sm.ns(Pseudotime, df = 3)", reducedModelFormulaStr = "~1", cores = detectCores(), relative_expr = F, weighted = F)
closeAllConnections()

standard_cds@dim_reduce_type <- 'ICA'
weihgted_std_AT12_cds_subset_all_gene <- branchTest(standard_cds[1:transcript_num, ], fullModelFormulaStr = "~sm.ns(Pseudotime, df = 3)", reducedModelFormulaStr = "~1", cores = detectCores(), relative_expr = F, weighted = T)
closeAllConnections()
weihgted_std_AT12_cds_subset_quake_gene <- branchTest(std_AT12_cds_subset_all_gene[add_quake_gene_all_marker_ids, ], fullModelFormulaStr = "~sm.ns(Pseudotime, df = 3)", reducedModelFormulaStr = "~1", cores = detectCores(), relative_expr = F, weighted = T)
closeAllConnections()
...
std_AT12_cds_subset_all_gene_ILRs_list <- calILRs(cds = std_AT12_cds_subset_all_gene[1:transcript_num, ], lineage_states = c(2, 3), stretch = T, cores = detectCores(), 
  ...
  useVST = F, round_exprs = FALSE, pseudocount = 0, output_type = "all", file = "std_AT12_cds_subset_all_gene_ILRs", return_all = T)
closeAllConnections()
...
duplicate_progenitors_weighted_std_AT12_cds_subset_all_gene_ILRs_list <- calILRs(cds = std_AT12_cds_subset_all_gene[1:transcript_num, ], lineage_states = c(2, 3), stretch = T, cores = detectCores(), 
    trend_formula = "~sm.ns(Pseudotime, df = 3) * Lineage", ILRs_limit = 3, progenitor_method = 'duplicate', 
    relative_expr = F, weighted = T, label_by_short_name = F, 
    useVST = F, round_exprs = FALSE, pseudocount = 0, output_type = "all", file = "duplicate_progenitors_weighted_std_AT12_cds_subset_all_gene_ILRs_list", return_all = T)
closeAllConnections()
